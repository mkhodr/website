...
def simulate(x=1000, y=500 ,particle_no=500, food_no=5000):
    ...
    def send_data():
        ## Particles 
        particle_info = [(p.position.x, p.position.y, p.size, p.velocity) for p in arena.particles]
        particle_positions_x, particle_positions_y, particle_size, particle_velocity = zip(*particle_info)
        ## Foods
        ...
        if food_info:
            ...
            return {'particles': {'x': list(particle_positions_x), 'y': list(particle_positions_y), 'r': list(particle_size), 'velocity': list(particle_velocity) },
    ...
    return {'particles': [[], [], []] , 'foods': [[], [], []], 'tick': ticks, 'runtime': run_time}
